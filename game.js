    ...
    generateCreatureActionsHTML(creature, index, location, maxLevel) {
        ...
        if (location === 'team') {
            ...
            if (this.playerTeam.length > 1) {
                actionsHTML += `<button class="btn" style="background: #007bff;" onclick="game.moveToStorage(${index}); game.closeCreatureModal();" ${isTowerActive ? 'disabled' : ''}>${isTowerActive ? 'VERROUILLÉ' : 'Stocker'}</button>`;
            }
            if (canUsePension && this.pension.length < maxPensionSlots && this.playerTeam.length > 1) {
                actionsHTML += `<button class="btn" style="background: #ff69b4;" onclick="game.moveToPension(${index}, true); game.closeCreatureModal();" ${isTowerActive ? 'disabled' : ''}>${isTowerActive ? 'VERROUILLÉ' : 'Pension'}</button>`;
            }
        } else if (location === 'storage') {
            if (this.playerTeam.length < maxTeamSize) {
                actionsHTML += `<button class="btn btn-save" onclick="game.moveToTeam(${index}); game.closeCreatureModal();" ${isTowerActive ? 'disabled' : ''}>${isTowerActive ? 'VERROUILLÉ' : 'Vers Équipe'}</button>`;
            }
            ...
            if (this.playerTeam.length < maxTeamSize) {
                actionsHTML += `<button class="btn btn-save" onclick="game.moveFromPension(${index}, true); game.closeCreatureModal();" ${isTowerActive ? 'disabled' : ''}>${isTowerActive ? 'VERROUILLÉ' : 'Vers Équipe'}</button>`;
            }
        ...
        if (canPrestige) {
            ...
            if (currentShards >= prestigeCost) {
                actionsHTML += `<button class="btn" style="background: #ffd700; color: #333;" onclick="game.prestigeCreature(${index}, '${location}'); game.closeCreatureModal();">⭐ PRESTIGE (${prestigeCost})</button>`;
            } else {
                actionsHTML += `<button class="btn" disabled style="background: #ccc; color: #666;">⭐ PRESTIGE (${currentShards}/${prestigeCost})</button>`;
            }
        }
    ...
    filteredDetails.forEach(fam => {
        ...
        html += `<div class="collection-synergy-card ${statusClass}" style="margin-bottom:20px; padding:15px; background:${fam.isActive ? 'rgba(34,197,94,0.06)' : 'rgba(0,0,0,0.03)'}; border-radius:12px; border:1px solid ${fam.isActive ? 'rgba(34,197,94,0.3)' : '#e2e8f0'};">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; flex-wrap:wrap; gap:8px;">
                <h4 style="margin:0; font-size:16px;">${fam.name}</h4>
                <div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
                    ${fam.bonusPerPrestige ? `<span style="font-size:11px; color:#64748b;" title="Bonus par niveau de prestige">${fam.bonusPerPrestige}</span>` : ''}
                    <span style="font-size:13px; color:${fam.isActive ? '#16a34a' : '#666'};">
        ...
        fam.members.forEach(m => {
            ...
            html += `<div class="creature-card ${rClass}" title="${title}" style="text-align:center; padding:4px; border-radius:8px; ${style}">
                ...
                <span style="font-size:10px; display:block; margin-top:2px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${m.name}</span>
                ${m.status === 'ok' ? `<span style="font-size:9px; color:#16a34a;">★${m.prestige}</span>` : (m.status === 'prestige_0' ? '<span style="font-size:9px; color:#ca8a04;">0</span>' : '<span style="font-size:9px; color:#999;">—</span>')}
            </div>`;
        });
        html += '</div></div>';
    });
